...
- [x] 2026-06-20: Edit/print bug cluster (v2 only):
  ...
  - **People on Saturday/last-Friday activities.** `EditSaturdayModal` activity rows now have the same "Who's joining?" picker as after-gan. On save, people resolve onto an activity row (find-or-create for free-text custom names, like the after-gan flow) → `associated_person_ids`, so chips render them via the adapter's `withSlugs`. People-less custom activities stay inline (no activity row created).
